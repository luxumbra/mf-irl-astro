...
export const useCountdown = (targetDate: string): CountdownReturnType => {
  ...
  const diff = countdownDate.diff(now, ['years', 'months', 'days', 'hours', 'minutes', 'seconds']);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const newDiff = countdownDate.diff(rightNow, ['years', 'months', 'days', 'hours', 'minutes', 'seconds']);
